...
const Header = () => {
  ...
  const { navigate, user, setUser, searchQuery, setSearchQuery, getCartCount, setShowUserLogin, logoutUser } =
      ...
      <header className="absolute top-0 left-0 right-0 max-padd-container flexBetween gap-4 py-2">
        ...
        <div className="flex sm:flex-1 items-center sm:justify-end gap-x-4 sm:gap-x-8">
          ...
          <div className="relative hidden xl:flex items-center">
            {/* Toggle input */}
            <div
              className={`bg-white ring-1 ring-slate-900/10 rounded-full overflow-hidden
          ...
          <Link to={"/cart"} className="flex relative">
            <div className="bold-16 ">
              Cart{" "}
              <span
                className="bg-secondary text-white text-[12px] font-semibold 
                absolute -top-3.5 -right-2 flexCenter w-4 h-4 rounded-full shadow-md"
              >
                {getCartCount()}
              </span>
            </div>
          ...
          <div className="group relative">
            <div className="">
              {user ? (
                ...
                <button onClick={()=> setShowUserLogin(true)}
                  className="btn-light flexCenter
                  gap-x-2"
                >
            ...
            {user && (
              <ul className="bg-white p-2 w-32 ring-1 ring-slate-900/5 rounded absolute right-0 top-10 hidden group-hover:flex flex-col medium-14 shadow-md z-50">
                ...
                <li onClick={logoutUser} className="p-2 rounded-md hover:bg-primary cursor-pointer">
                  Logout
                </li>
              </ul>
            )}
